document.addEventListener('DOMContentLoaded', function() {
  ...
  // --- FingerprintJS Initialization (Added for Tip Button) ---
...
if (isDevEnv) console.log('Lightbox script initialized');
  ...
  // --- Search Section Functionality (Updated to use getFingerprint) ---
  ...
  searchIcon.addEventListener('click', () => {
    if (searchContainer.classList.contains('hidden')) {
      ...
      hide(searchIconContainer);
      searchInput.focus();
    } else {
      hide(searchContainer);
      show(searchIconContainer);
  ...
  document.addEventListener('click', (e) => {
    if (
      ...
      !searchIconContainer.contains(e.target) &&
  ...
  async function fetchSearchResults(query) {
    try {
      ...
      const response = await fetch(`/home/search-creators?query=${encodeURIComponent(query)}&fingerprint=${encodeURIComponent(fingerprintId)}`, {
        credentials: 'include'
      });
      if (!response.ok) {
        if (response.status === 429) {
          if (isDevEnv) console.log('Rate limit exceeded for search-creators');
          throw new Error('Too many requests. Please try again in a minute.');
        }
        throw new Error(`Fetch error: ${response.status}`);
      }
      const data = await response.json();
      if (data.message && !data.creators.length) {
        throw new Error(data.message);
      }

      hide(feedModeContainer);
      show(searchResultsContainer);

      const resultsHTML = data.creators.length
        ? `<h1>Search Results for "${query}"</h1>` +
          data.creators.map(creator => `
            <div class="creator-result">
              <img src="${creator.profilePicture || '/Uploads/default-profile-picture.jpg'}" alt="${creator.username}" class="creator-profile-pic-md" />
      ...
      if (isDevEnv) console.error('Error fetching search results:', err.message);
      searchResultsContainer.innerHTML = `<p>Error fetching search results: ${err.message}</p>`;
    }
  }
  ...
  searchInput.addEventListener('input', () => {
    ...
    debounceTimeout = setTimeout(async () => {
      try {
        const fingerprintId = await getFingerprint();
        const response = await fetch(`/home/search-suggestions?query=${encodeURIComponent(query)}&fingerprint=${encodeURIComponent(fingerprintId)}`, {
          credentials: 'include'
        });
        if (!response.ok) {
          if (response.status === 429) {
            if (isDevEnv) console.log('Rate limit exceeded for search-suggestions');
            throw new Error('Too many requests. Please try again in a minute.');
          }
          throw new Error(`Fetch error: ${response.status}`);
        }
        ...
        const html = data.creators.length
          ? data.creators.map(c => `
              <div class="creator-result suggestion-item" data-user-id="${c._id}">
                <img src="${c.profilePicture || '/Uploads/default-profile-picture.jpg'}" alt="${c.username}" class="creator-profile-pic-sm" />
                <div class="creator-info">
                  <a href="/profile/view/${c._id}" class="suggestion-link"><strong>${c.username}</strong></a>
                  ${c.profileName ? `<p>${c.profileName}</p>` : ''}
                  <p>${c.subscriberCount || 0} subscriber(s)</p>
                </div>
              </div>
            `).join('')
        ...
        document.querySelectorAll('.suggestion-item').forEach(item => {
          item.addEventListener('click', e => {
            e.preventDefault();
            window.location.href = `/profile/view/${item.dataset.userId}`;
          });
        });
      } catch (err) {
        if (isDevEnv) console.error('Error fetching suggestions:', err.message);
        suggestionsContainer.innerHTML = `<div class="no-suggestions">Error fetching suggestions: ${err.message}</div>`;
        show(suggestionsContainer);
      }
    }, 300);
  });
  ...
  // --- Unlock Special Content (Updated) ---
...
reportForm.addEventListener('submit', async function(e) {
  ...
  const trendingList = document.querySelector('.trending-creators-list');
  const dots = document.querySelectorAll('.slider-dots .dot');
  const trendingContainer = document.querySelector('.trending-creators-container');
  const totalSlides = Math.ceil(document.querySelectorAll('.featured-creator-card').length / 4);
  let autoSlideInterval;

  // Function to update the active dot based on scroll position
  function updateActiveDot() {
    const scrollLeft = trendingList.scrollLeft;
    const slideWidth = trendingList.querySelector('.creator-slide').offsetWidth + 20; // Include gap
  ...
  trendingList.addEventListener('scroll', updateActiveDot);

  // Function to start auto-sliding
  function startAutoSlide() {
    autoSlideInterval = setInterval(() => {
      const slideWidth = trendingList.querySelector('.creator-slide').offsetWidth + 20; // Include gap
      const nextScrollLeft = trendingList.scrollLeft + slideWidth;
      if (nextScrollLeft + trendingList.clientWidth >= trendingList.scrollWidth) {
        trendingList.scrollTo({ left: 0, behavior: 'smooth' }); // Loop back to start
      } else {
        trendingList.scrollTo({ left: nextScrollLeft, behavior: 'smooth' });
      }
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', () => {
      ...
      const slideWidth = trendingList.querySelector('.creator-slide').offsetWidth + 20; // Include gap
      trendingList.scrollTo({ left: index * slideWidth, behavior: 'smooth' });
    });
  });

  // Pause and resume auto-slide on hover
  trendingContainer.addEventListener('mouseenter', stopAutoSlide);
  trendingContainer.addEventListener('mouseleave', startAutoSlide);
  ...
  if (isDevEnv && typeof fetchWithCsrf !== 'function') {
    console.error('fetchWithCsrf is not defined. Ensure utils.js is loaded before home.js');
  }
});