...
if (isDevEnv) console.log('Lightbox script initialized');
  ...
  // --- Search Section Functionality (Refactored) ---
...
async function fetchSearchResults(query) {
  try {
    const response = await fetch(`/home/search-creators?query=${encodeURIComponent(query)}`, { credentials: 'include' });
...
searchInput.addEventListener('input', () => {
  ...
  debounceTimeout = setTimeout(async () => {
    try {
      const response = await fetch(`/home/search-suggestions?query=${encodeURIComponent(query)}`, { credentials: 'include' });
      if (!response.ok) throw new Error(`Fetch error: ${response.status}`);
      const data = await response.json();
  ...
  document.querySelectorAll('.like-button').forEach(button => {
    button.addEventListener('click', async function() {
      ...
      try {
        ...
        const res = await fetchWithCsrf('/profile/posts/' + postId + '/like', { method: 'POST' });
  ...
  document.querySelectorAll('.bookmark-button').forEach(button => {
    button.addEventListener('click', async function() {
      ...
      try {
        ...
        const res = await fetchWithCsrf('/profile/posts/' + postId + '/bookmark', { method: 'POST' });
        const data = await res.json();
        if (res.ok) {
